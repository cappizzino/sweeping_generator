...
class Node:
    ...
    def __init__(self):
        ...
        self.sub_odom = rospy.Subscriber(f"/{self.leader_name}/estimation_manager/odom_main", Odometry, self.callbackOdom)
        self.sub_mpc = rospy.Subscriber(f"/{self.leader_name}/control_manager/mpc_tracker/predicted_trajectory_debugging", PoseArray, self.callbackMPC)

        ## | ----------------------- publishers ---------------------- |

        self.ref_pub = rospy.Publisher(f"/{self.leader_name}/leader_reference", Reference, queue_size=1)
